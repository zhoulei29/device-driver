	...
	int id, struct uvc_entity *entity)
{
	unsigned int i;
	/*
	list_entry()这个函数表示遍历整个链表，其中第一个参数为指向链表头的指针，第二个
	为结构体的类型，第三个为成员变量名
	*/
	if (entity == NULL)
		entity = list_entry(&dev->entities, struct uvc_entity, list);
	/*
	遍历不是从链表头开始，而是从已知的某个节点pos开始，则可以使
	用list_for_each_entry_continue(pos,head,member)。
	*/
...
/*这个函数功能为：将有相同uvc视频流ID号的uvc流结构体返回，否则返回NULL*/
...
{
	/*
	依然是通过struct uvc_entity 这个实体结构体中的type成员来判断实体的类型，
	使用 0x7fff 作为掩码检验的方式
	*/
	/*
	switch 代码段的分析：
	1.首先判断实体的类型（ 例如：case UVC_VC_EXTENSION_UNIT: ）
	2.判断用户是否设置了uvc_trace_param这个参数，且参数值为UVC_TRACE_PROBE，
	如果设置了则打印相关信息
	3.
	*/
	...
	case UVC_VC_PROCESSING_UNIT:
		...
		/*如果实体为处理unit时，则将此实体指向uvc视频链的processing成员*/
	...
	case UVC_VC_SELECTOR_UNIT:
		...
		/*如果实体为选择unit时，则将此实体指向uvc视频链的selector成员*/
	...
	case UVC_ITT_CAMERA:
	/*input Terminal Type*/
	case UVC_ITT_MEDIA_TRANSPORT_INPUT://media
		if (uvc_trace_param & UVC_TRACE_PROBE)
			printk(" <- IT %d\n", entity->id);
	...
	case UVC_TT_STREAMING://streaming
		/*
		UVC_ENTITY_IS_ITERM()判断实体是否为input Terminal,判断方法任然是使用
		0x8000 掩码完成的
		*/
	...
	struct uvc_video_chain *chain)
{
	...
	list_for_each_entry(term, &chain->entities, chain) {
		/*如果判断此 Terminal      的类型不是streaming的话则重新遍历*/
		if (UVC_ENTITY_TYPE(term) != UVC_TT_STREAMING)
			continue;
		/*返回具有相同id的uvc流结构体（struct uvc_streaming)*/
...
static void uvc_disconnect(struct usb_interface *intf)
{
	 ...
	 */
	 /*
	 和普通的usb设备相同，都需要将保存在 
	 struct usb_interface-->struct device-->struct device_private-->void *driver_data
	 的结构体指向NULL
	 */
	...
	if (intf->cur_altsetting->desc.bInterfaceSubClass ==
	    UVC_SC_VIDEOSTREAMING)
		return;
	/*需要在struct uvc_device --> enum uvc_device_state state中的保存断开连接的状态*/
...
/*
[]MODULE_PARM_DESC()被用来注解该模块可以接收的参数。该宏 两个参数：变量名
和一个格式自由的对该变量的描述
*/
MODULE_PARM_DESC(clock, "Video buffers timestamp clock");
/*
module_param_named(name, variable, type, perm);其中name是外部可见的参数名，
variable是源文件内部的全局变量名，而module_param通过module_param_named实现，
只不过name与variable相同。
*/
...
{
	int result;
	/*struct uvc_driver --> srtuct usb_driver 来使用usb_register()函数注册USB驱动*/
